...
for (i in 1:5){legend("topleft",c("SCAT13","SCAT8","SCAT4","SCAT1","oracle"),bty="n",pch=c(15:19),col=c(1:(p+1))+1,cex=1)}
...
fdr = function(x) {p.adjust(x, method ="bonferroni")}
res1=cbind(scat1,scat2,scat3,scat4,ux)
p=dim(res1)[2]
px=matrix(NA,length(tx),p)
for (j in 1:length(tx))
{
